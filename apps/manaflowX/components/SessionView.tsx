...
function PartRenderer({ part }: { part: Part }) {
  switch (part.type) {
    ...
    case "tool_call":
      return (
        <div className="my-2 bg-gray-800 rounded-lg p-3 border border-gray-700">
...
export function SessionView({ sessionId }: { sessionId: Id<"sessions"> }) {
  ...
  return (
    <div className="border border-gray-800 rounded-lg overflow-hidden">
      ...
      <div className="divide-y divide-gray-800">
        {turns.map((turn) => (
          <TurnView key={turn._id} turn={turn as Turn} />
        ))}
      </div>
    </div>
  );
...
export function SessionsByPost({ postId }: { postId: Id<"posts"> }) {
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {sessions.map((session) => (
        <SessionView key={session._id} sessionId={session._id} />
      ))}
    </div>
  );
}

...
/**
 * Link a coding agent session to its parent tool call and store the JWT secret.
 * Called from the coding agent tool when the session is created.
 */
...
export const getJwtSecretForSession = query({
  ...
  handler: async (ctx, args) => {
    const toolCall = await ctx.db
      .query("codingAgentToolCalls")
      .withIndex("by_coding_agent_session", (q) => q.eq("codingAgentSessionId", args.sessionId))
      .first();

    return toolCall?.jwtSecret ?? null;
  },
});

...
export type UseTaskCreationFormOptions = {
  teamSlugOrId: string;
  searchEnvironmentId?: string;
  enableGlobalKeydown?: boolean;
};

export type TaskCreationFormState = {
  ...
  shouldShowWorkspaceSetup: boolean;
};
...
export function useTaskCreationForm({
...
}: UseTaskCreationFormOptions): TaskCreationFormState {
  ...
  useEffect(() => {
    if (searchEnvironmentId) {
      const val = `env:${searchEnvironmentId}`;
  ...
  }, [searchEnvironmentId, teamSlugOrId]);
  ...
  const createTask = useMutation(api.tasks.create).withOptimisticUpdate(
    (localStore, args) => {
      ...
      if (currentTasks !== undefined) {
        ...
        localStore.setQuery(api.tasks.get, { teamSlugOrId }, [
          optimisticTask,
          ...currentTasks,
        ]);
      }
  ...
  const handleStartTask = useCallback(async (): Promise<boolean> => {

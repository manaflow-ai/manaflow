...
export function PrScreenshotGallery({ screenshotSets }: PrScreenshotGalleryProps) {
  ...
  // Flatten all images across sets for navigation
  ...
  const currentEntryKey = currentEntry?.key;
  useEffect(() => {
    defaultZoomRef.current = 1;
    defaultOffsetRef.current = { x: 0, y: 0 };
    setBaseImageScale(1);
    resetZoomState({ zoom: 1, offset: { x: 0, y: 0 } });
  }, [currentEntryKey, resetZoomState]);
  ...
  // Don't render anything if there are no screenshot sets
  if (sortedScreenshotSets.length === 0) {
    return null;
  }

  // Get the latest set for highlighting
  const latestSetId = sortedScreenshotSets[0]?._id ?? null;

  // Count total images
  const totalImages = flattenedImages.length;

  return (
    <div className="rounded-lg border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-950/70">
      ...
      <div className="p-3 space-y-3">
        ...
        {currentEntry ? (
          ...
          >
            <Dialog.Portal>
              ...
              <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[calc(100vh-4rem)] w-[min(2600px,calc(100vw-4rem))] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-xl border border-neutral-200 bg-white/95 p-4 shadow-2xl focus:outline-none dark:border-neutral-800 dark:bg-neutral-950/95 sm:max-h-[calc(100vh-6rem)] sm:w-[min(2600px,calc(100vw-6rem))] sm:p-6">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1 rounded-full border border-neutral-200 bg-white/90 px-2 py-1 text-xs font-medium text-neutral-600 shadow-sm dark:border-neutral-700 dark:bg-neutral-900/70 dark:text-neutral-200">
                      <button
                        ...
                        disabled={!canZoomOut}
                        className="rounded-full p-1 transition disabled:opacity-40 hover:bg-neutral-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/70 dark:hover:bg-neutral-800/80"
                        aria-label="Zoom out"
                      >
                ...
                <div className="flex flex-1 items-center gap-4">
                  ...
                  <div
                    ...
                    className={cn(
                      "relative flex h-[70vh] max-h-[calc(100vh-10rem)] min-h-[360px] w-full flex-1 items-center justify-center overflow-hidden rounded-lg border border-neutral-200 bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900",
                      zoom > 1
                        ? isPanning
                          ? "cursor-grabbing"
                          : "cursor-grab"
                      ...
                      {flattenedImages.map((entry) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "group relative flex h-20 w-32 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg border border-neutral-200 bg-neutral-50 p-1 transition hover:border-neutral-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/70 dark:border-neutral-700 dark:bg-neutral-900/70 dark:hover:border-neutral-500",
                              isActiveThumb &&
                                "border-sky-400/70 shadow-[0_0_0_1px_rgba(56,189,248,0.25)] dark:border-sky-400/60"
                            )}
        ...
        {sortedScreenshotSets.map((set) => {
          ...
          const isLatest = latestSetId === set._id;

          return (
            <div
              key={set._id}
              className={cn(
                "rounded-lg border border-neutral-200 bg-neutral-50/50 p-2 dark:border-neutral-800 dark:bg-neutral-900/50",
                isLatest &&
                  "border-sky-300/70 dark:border-sky-500/50"
              )}
            >
              <div className="flex flex-wrap items-center gap-1.5 text-xs">
                {isLatest && (
                  <span className="px-1.5 py-0.5 font-medium rounded bg-sky-100 text-sky-700 dark:bg-sky-900/60 dark:text-sky-300">
                    Latest
                  </span>
                )}
                <span
                  ...
                  {set.images.map((image, indexInSet) => {
                    ...
                    if (!image.url) {
                      return (
                        <div
                          key={stableKey}
                          className="flex h-24 min-w-[100px] items-center justify-center rounded border border-dashed border-neutral-300 bg-neutral-100 text-[10px] text-neutral-500 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400"
                        >
                          Unavailable
                        </div>
                      );
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "group relative flex h-24 w-[100px] flex-shrink-0 overflow-hidden rounded border border-neutral-200 bg-white transition-colors hover:border-neutral-400 dark:border-neutral-700 dark:bg-neutral-900/70 dark:hover:border-neutral-500",
                          isActive &&
                            "border-sky-400/70 shadow-[0_0_0_1px_rgba(56,189,248,0.25)] dark:border-sky-400/60"
                        )}
                      ...
                      >
                        <img
                          ...
                          loading="lazy"
                        />
                        <div className="absolute inset-0 flex items-center justify-center bg-neutral-950/0 opacity-0 transition group-hover:bg-neutral-950/30 group-hover:opacity-100">
                          <Maximize2 className="h-4 w-4 text-white" />
                        </div>
                        {humanIndex !== null && (
                          <span className="pointer-events-none absolute bottom-1 left-1 rounded bg-neutral-950/70 px-1 text-[9px] font-medium text-white">
                            {humanIndex}
                          </span>
                        )}
                      </button>

...
type ScreenshotSet = NonNullable<
  FunctionReturnType<typeof api.previewRuns.listScreenshotSetsForPr>
>[number];

type ScreenshotImage = ScreenshotSet["images"][number];
...
export function PrScreenshotGallery(props: PrScreenshotGalleryProps) {
  const { teamSlugOrId, repoFullName, prNumber } = props;

  const queryArgs = useMemo(
    () =>
      teamSlugOrId && repoFullName && prNumber
        ? { teamSlugOrId, repoFullName, prNumber, limit: 5 }
        : "skip",
    [teamSlugOrId, repoFullName, prNumber]
  );
  ...
  // Don't render anything if no data or empty array
  if (!screenshotSets || screenshotSets.length === 0) {
    return null;
  }

  return (
    <PrScreenshotGalleryContent screenshotSets={screenshotSets} />
  );
}

function PrScreenshotGalleryContent({
  screenshotSets,
}: {
    ...
    () =>
      [...screenshotSets].sort((a, b) => {
        if (a.capturedAt === b.capturedAt) {
          return a.screenshotSetId.localeCompare(b.screenshotSetId);
        }
        return b.capturedAt - a.capturedAt; // Most recent first
      }),
    [screenshotSets]
  );
    ...
    sortedScreenshotSets.forEach((set) => {
      set.images.forEach((image, indexInSet) => {
        ...
        entries.push({
          ...
          image,
          indexInSet,
          key: getImageKey(set.screenshotSetId, image, indexInSet),
  ...
  // Only render the most recent screenshot set for compactness in the sidebar
  const latestSet = sortedScreenshotSets[0];
  if (!latestSet) {
    return null;
  }

  const totalImages = sortedScreenshotSets.reduce(
    (acc, set) => acc + set.images.length,
    0
  );

  return (
    <div className="border border-neutral-200 bg-white p-5 pt-4">
      ...
      {currentEntry ? (
        ...
        >
          <Dialog.Portal>
            <Dialog.Overlay className="fixed inset-0 bg-neutral-950/60 z-50 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in data-[state=closed]:fade-out" />
            <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[calc(100vh-4rem)] w-[min(2600px,calc(100vw-4rem))] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 border border-neutral-200 bg-white/95 p-4 shadow-2xl focus:outline-none sm:max-h-[calc(100vh-6rem)] sm:w-[min(2600px,calc(100vw-6rem))] sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="space-y-1">
                  ...
                  <Dialog.Description className="text-xs text-neutral-600">
                    ...
                    {formatDistanceToNow(
                      new Date(currentEntry.set.capturedAt),
                      {
                        addSuffix: true,
                      }
                    )}
                    {currentEntry.set.headSha && (
                      <>
                        <span className="px-1 text-neutral-400">•</span>
                        <span className="font-mono">
                          {currentEntry.set.headSha.slice(0, 7)}
                        </span>
                      </>
                    )}
                  </Dialog.Description>
                    ...
                    {flattenedImages.map((entry) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "group relative flex h-20 w-32 flex-shrink-0 items-center justify-center overflow-hidden border border-neutral-200 bg-neutral-50 p-1 transition hover:border-neutral-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/70",
                            isActiveThumb &&
                              "border-sky-400/70 shadow-[0_0_0_1px_rgba(56,189,248,0.25)]"
                          )}
      ...
      {/* Screenshot thumbnails in sidebar - heatmap style */}
      <div className="space-y-3">
        {sortedScreenshotSets.slice(0, 3).map((set) => {
          const capturedAtDate = new Date(set.capturedAt);
          const relativeCapturedAt = formatDistanceToNow(capturedAtDate, {
            addSuffix: true,
          });
          const shortCommit = set.headSha?.slice(0, 7);
          const isLatest = set.screenshotSetId === latestSet.screenshotSetId;

          return (
            <article
              key={set.screenshotSetId}
              className={cn(
                "border border-neutral-200 bg-white p-3 transition-shadow",
                isLatest &&
                  "border-sky-200 shadow-[0_0_0_1px_rgba(56,189,248,0.15)]"
              )}
            >
              <div className="flex flex-wrap items-center gap-2 text-xs">
                <span
                  className={cn(
                    ...
                    STATUS_STYLES[set.screenshotStatus] ?? STATUS_STYLES.completed
                  )}
                >
                  {STATUS_LABELS[set.screenshotStatus] ?? "Unknown"}
                </span>
                {isLatest && (
                  <span className="px-2 py-0.5 font-medium bg-sky-100/80 text-sky-700">
                    Latest
                  </span>
                )}
                <span
              ...
              {set.images.length > 0 ? (
                <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
                  {set.images.slice(0, 4).map((image, indexInSet) => {
                    const displayName = image.fileName ?? "Screenshot";
                    const stableKey = getImageKey(
                      set.screenshotSetId,
                      image,
                      indexInSet
                    );
                    if (!image.url) {
                      return (
                        <div
                          key={stableKey}
                          className="flex h-24 min-w-[100px] items-center justify-center border border-dashed border-neutral-300 bg-neutral-100 text-xs text-neutral-500"
                        >
                          URL expired
                        </div>
                      );
                    ...
                    const humanIndex =
                      flatIndex !== null ? flatIndex + 1 : null;
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "group relative flex w-[120px] flex-col overflow-hidden border border-neutral-200 bg-neutral-50 text-left transition-colors hover:border-neutral-400",
                          isActive &&
                            "border-sky-400/70 shadow-[0_0_0_1px_rgba(56,189,248,0.25)]"
                        )}
                      ...
                      >
                        <img
                          src={image.url}
                          alt={displayName}
                          className="h-24 w-[120px] object-contain bg-neutral-100"
                          loading="lazy"
                        />
                        <div className="absolute top-1 right-1 text-neutral-600 opacity-0 transition group-hover:opacity-100">
                          <Maximize2 className="h-3 w-3" />
                        </div>
                        {humanIndex !== null && (
                          <span className="pointer-events-none absolute bottom-1 left-1 bg-neutral-950/80 px-1 text-[9px] font-semibold text-white">
                            {humanIndex}
                          </span>
                        )}
                      </button>
                  ...
                  {set.images.length > 4 && (
                    <div className="flex h-24 min-w-[60px] items-center justify-center border border-neutral-200 bg-neutral-100 text-xs text-neutral-500">
                      +{set.images.length - 4}
                    </div>
                  )}
                </div>
        ...
        {sortedScreenshotSets.length > 3 && (
          <p className="text-xs text-neutral-500 text-center">
            +{sortedScreenshotSets.length - 3} more screenshot{" "}
            {sortedScreenshotSets.length - 3 === 1 ? "set" : "sets"}
          </p>
        )}
      </div>
    </div>

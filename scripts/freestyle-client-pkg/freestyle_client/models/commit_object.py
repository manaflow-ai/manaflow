...
class CommitObject(BaseModel):
    ...
    Commit object
    """ # noqa: E501
    author: Signature = Field(description="The author of the commit")
    committer: Signature = Field(description="The committer")
    message: StrictStr = Field(description="The commit message")
    tree: CommitTree = Field(description="The ID of the tree pointed to by this commit")
    parents: List[CommitParent] = Field(description="Parent commit(s) of this commit")
    sha: StrictStr = Field(description="The commit's hash ID")
    __properties: ClassVar[List[str]] = ["author", "committer", "message", "tree", "parents", "sha"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "author": Signature.from_dict(obj["author"]) if obj.get("author") is not None else None,
            "committer": Signature.from_dict(obj["committer"]) if obj.get("committer") is not None else None,
            "message": obj.get("message"),
            "tree": CommitTree.from_dict(obj["tree"]) if obj.get("tree") is not None else None,
            "parents": [CommitParent.from_dict(_item) for _item in obj["parents"]] if obj.get("parents") is not None else None,
            "sha": obj.get("sha")
        })
        return _obj

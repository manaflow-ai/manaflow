...
class VmPersistenceOneOf(BaseModel):
    """
    When your storage quota is reached, the least recently used VMs will be deleted.
    """ # noqa: E501
    priority: Optional[Annotated[int, Field(le=10, strict=True, ge=0)]] = Field(default=5, description="Priority for eviction when storage quota is reached. Higher values mean the VM is less likely to be evicted. Range is 0-10, default is 5.")
    type: StrictStr
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['sticky']):
            raise ValueError("must be one of enum values ('sticky')")
        return value

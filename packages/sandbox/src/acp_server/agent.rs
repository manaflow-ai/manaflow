...
/// A no-op Client implementation for the ACP server.
/// This is used when creating a ClientSideConnection to the spawned CLI.
...
impl agent_client_protocol::Client for NoOpClient {
    ...
    ) -> Result<agent_client_protocol::RequestPermissionResponse, agent_client_protocol::Error>
    {
        ...
        Ok(agent_client_protocol::RequestPermissionResponse {
            outcome: agent_client_protocol::RequestPermissionOutcome::Selected {
                option_id: agent_client_protocol::PermissionOptionId("allow".into()),
            },
            meta: None,
        })
    }
    ...
    async fn read_text_file(
        &self,
        request: agent_client_protocol::ReadTextFileRequest,
        ...
        match tokio::fs::read_to_string(&request.path).await {
            Ok(content) => Ok(agent_client_protocol::ReadTextFileResponse {
                content,
                meta: None,
            }),
            Err(e) => Err(agent_client_protocol::Error::internal_error().with_data(e.to_string())),
        }
    }

    async fn write_text_file(
        &self,
        request: agent_client_protocol::WriteTextFileRequest,
        ...
        match tokio::fs::write(&request.path, &request.content).await {
            Ok(_) => Ok(agent_client_protocol::WriteTextFileResponse::default()),
            Err(e) => Err(agent_client_protocol::Error::internal_error().with_data(e.to_string())),
        }
    }
    ...
    async fn create_terminal(
        ...
        Err(agent_client_protocol::Error::internal_error().with_data("Terminal not supported"))
    }
    ...
    async fn session_notification(
        ...
        // Handle session notifications (agent messages, tool calls, etc.)
        // This will be used to persist streaming updates to Convex
        Ok(())
    }
}

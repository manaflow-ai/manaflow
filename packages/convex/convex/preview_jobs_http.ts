...
export const completePreviewJob = httpAction(async (ctx, req) => {
  ...
  try {
    ...
    if (previewRun.repoInstallationId) {
      ...
      // Check if there's an existing preliminary comment to update
      // If we posted a preliminary comment earlier, update it with screenshots
      // Otherwise, create a new comment (fallback for older flows)
      let commentResult: { ok: true; commentUrl?: string } | { ok: false; error: string };

      if (previewRun.githubCommentId) {
        // Update the existing preliminary comment with screenshots
        console.log("[preview-jobs-http] Updating existing preliminary comment", {
          taskRunId,
          previewRunId: previewRun._id,
        ...
        // No existing comment - create a new one (fallback path)
        console.log("[preview-jobs-http] No existing comment, creating new one", {
          taskRunId,
          previewRunId: previewRun._id,
        });

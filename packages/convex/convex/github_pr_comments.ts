...
export const postPreviewCommentWithTaskScreenshots = internalAction({
  ...
  handler: async (ctx, args): Promise<{ ok: true; commentId?: number; commentUrl?: string } | { ok: false; error: string }> => {
    ...
    try {
      ...
      if (!taskRun) {
        console.error("[github_pr_comments] Task run not found", {
          taskRunId,
        });
      ...
      // Get screenshot set from task run - try latestScreenshotSetId first, fall back to query
      let screenshotSet = taskRun.latestScreenshotSetId
        ? await ctx.runQuery(
            internal.github_pr_queries.getScreenshotSet,
      ...
      // If no latestScreenshotSetId (e.g., skipped screenshots), query for the latest set
      if (!screenshotSet) {
        screenshotSet = await ctx.runQuery(
      ...
      if (!screenshotSet) {
        console.error("[github_pr_comments] Screenshot set not found", {
          taskRunId,
          latestScreenshotSetId: taskRun.latestScreenshotSetId,
        });
      ...
      if (screenshotSet.status === "completed" && screenshotSet.images.length > 0) {
        ...
        if (isNonUiChangeReason(screenshotSet.error)) {
          commentBody += `⏭️ **No UI-impacting changes detected**\n\n`;
          commentBody += `Screenshot capture was skipped because no UI-impacting changes were found in this PR.\n\n`;
          if (screenshotSet.error) {
            commentBody += `> ${screenshotSet.error}\n\n`;
          }
        } else {
          commentBody += `⏭️ **Screenshot capture was skipped**\n\n`;
          commentBody += `${screenshotSet.error || "No preview configured"}\n\n`;
        }
        commentBody += `---\n_Generated by [cmux](https://cmux.dev) preview system_`;
      }
